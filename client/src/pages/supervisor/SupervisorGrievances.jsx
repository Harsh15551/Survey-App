...
export default function SupervisorGrievances() {
  const { user } = useAuth()
  const { t } = useLanguage()
  ...
  async function triggerExport(type) {
    try {
      ...
      if (type === 'csv') exportGrievancesToCsv(list, `team-grievances-${user.region}.csv`)
